...
early_stop = EarlyStopping( #ensures model stops early if accuracy does not improve for 5 iterations
    monitor='val_loss',
    patience=5,           
    restore_best_weights=True 
)

reg = regularizers.l2(regularizer_val)
...
print("Evaluation of RNN stock prediction for " + company_name)
print("----------------------------------------------------")
...
plt.xlabel('Time')
plt.ylabel('Price')
plt.title("Actual vs predicted stock prices of " + company_name + " using RNN prediction")
plt.legend()
plt.show()
...
if len(sys.argv)>=4 and sys.argv[3] == "True":
    ...
    #INITIALIZE RNN EXPLAINER

    IG_explainer = IntegratedGradients(model, layer=None)
