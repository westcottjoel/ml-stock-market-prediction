...
#TUNING MODEL TAKES IN 2 CMDLINE ARGUMENTS -
...
def model_builder(hp):
    ...
    model.add(tfkl.LSTM(units=hp_units,return_sequences=True, input_shape=(train_set_x.shape[1], train_set_x.shape[2])))
    model.add(tfkl.Dropout(hp_dropout))
    model.add(tfkl.LSTM(units=hp_units,return_sequences=True))
    model.add(tfkl.Dropout(hp_dropout))
    model.add(tfkl.LSTM(units=hp_units))
    model.add(tfkl.Dropout(hp_dropout))
